...
- The shared `OpenClaw` request contract now carries a typed live fetch-entry record so execution bridges resolve into one network-invocation-boundary placeholder shape without network calls.
...
- The live path now resolves a shared fetch-execution record and surfaces that transport-implementation-boundary placeholder posture through diagnostics, HUD provider metadata, and `Provider Status` details before actual live network transport implementation exists.
- The live path now resolves a shared execution-delegate record and surfaces that network-runner-boundary placeholder posture through diagnostics, HUD provider metadata, and `Provider Status` details before actual live network transport runners exist.
- The live path now resolves a shared transport-implementation record and surfaces that fetch-runner-boundary placeholder posture through diagnostics, HUD provider metadata, and `Provider Status` details before actual live network execution exists.
- The live path now resolves a shared transport-runner record and surfaces that final pre-network execution posture through diagnostics, HUD provider metadata, and `Provider Status` details before any real HTTP execution exists.
- The live path now resolves a shared network-execution seam and routes preview/live-stub runner behavior through that seam while surfacing its metadata through diagnostics, HUD provider metadata, and `Provider Status` details before any real HTTP execution exists.
- The live path now resolves a shared request-dispatch handler and routes network execution through that handler while surfacing its metadata through diagnostics, HUD provider metadata, and `Provider Status` details before any real HTTP execution exists.
- The live path now resolves a shared HTTP-bridge handler and routes request dispatch through that bridge while surfacing its metadata through diagnostics, HUD provider metadata, and `Provider Status` details before any real HTTP execution exists.
- The live path now resolves a shared transport-callable handler and routes the HTTP bridge through that callable while surfacing its metadata through diagnostics, HUD provider metadata, and `Provider Status` details before any real HTTP execution exists.
