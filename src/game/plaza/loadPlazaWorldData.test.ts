...
test("loadPlazaWorldData passes through successful provider results", async () => {
  ...
  expect(world.hotspots[0]?.details).toContain(
    "Execution bridge: OpenClaw live execution bridge"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Execution bridge status: Preview payload"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Bridge payload: Preview payload available from no-network live-preview execution."
  );
  expect(world.hotspots[0]?.details).toContain(
    "Bridge source payload: Preview payload from GET Configured via future live request seam?view=plaza&workspace=mii-plaza-client"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Bridge target: OpenClaw live fetch entrypoint"
  );
  expect(world.hotspots[0]?.details).toContain("Bridge mode: Preview");
  expect(world.hotspots[0]?.details).toContain(
    "Fetch entry: OpenClaw live fetch entry"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Fetch entry status: Preview payload"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Entry payload: Preview payload available from no-network live-preview execution."
  );
  expect(world.hotspots[0]?.details).toContain(
    "Entry source bridge: Preview payload from GET Configured via future live request seam?view=plaza&workspace=mii-plaza-client"
  );
  expect(world.hotspots[0]?.details).toContain(
  ...
  expect(world.hotspots[0]?.details).toContain(
    "Transport call: OpenClaw live transport call"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Transport call status: Preview payload"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Transport payload: Preview payload available from no-network live-preview execution."
  );
  expect(world.hotspots[0]?.details).toContain(
    "Transport source dispatch: Preview payload from GET Configured via future live request seam?view=plaza&workspace=mii-plaza-client"
  );
  expect(world.hotspots[0]?.details).toContain(
  ...
  expect(world.hotspots[0]?.details).toContain("Execution mode: Preview");
  expect(world.hotspots[0]?.details).toContain(
    "Execution delegate: OpenClaw live execution delegate"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Execution delegate status: Preview payload"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Delegate payload: Preview payload available from no-network live-preview execution."
  );
  expect(world.hotspots[0]?.details).toContain(
    "Delegate source execution: Preview payload from GET Configured via future live request seam?view=plaza&workspace=mii-plaza-client"
  );
  expect(world.hotspots[0]?.details).toContain(
  ...
  expect(world.hotspots[0]?.details).toContain(
    "Transport runner: OpenClaw live transport runner"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Transport runner status: Preview payload"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Runner payload: Preview payload available from no-network live-preview execution."
  );
  expect(world.hotspots[0]?.details).toContain(
    "Runner source implementation: Preview payload from GET Configured via future live request seam?view=plaza&workspace=mii-plaza-client"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Runner target: OpenClaw live network execution"
  );
  expect(world.hotspots[0]?.details).toContain("Runner mode: Preview");
  expect(world.hotspots[0]?.details).toContain(
    "Network execution: OpenClaw live network execution"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Network execution status: Preview payload"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Network payload: Preview payload available from no-network live-preview execution."
  );
  expect(world.hotspots[0]?.details).toContain(
    "Network source runner: Preview payload from GET Configured via future live request seam?view=plaza&workspace=mii-plaza-client"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Network implementation: Preview payload injection selected without network I/O."
  );
  expect(world.hotspots[0]?.details).toContain("Network mode: Preview");
  expect(world.hotspots[0]?.details).toContain(
    "Request dispatch: OpenClaw live request dispatch"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Request dispatch status: Preview payload"
  );
  expect(world.hotspots[0]?.details).toContain(
    "Dispatch payload: Preview payload available from no-network live-preview execution."
  );
  expect(world.hotspots[0]?.details).toContain(
    "Dispatch source execution: Preview payload from GET Configured via future live request seam?view=plaza&workspace=mii-plaza-client"
  );
  expect(world.hotspots[0]?.details).toContain(
  ...
  expect(world.hotspots[0]?.details).toContain("Dispatch mode: Preview");
  expect(world.hotspots[0]?.details).toContain(
    "HTTP bridge: OpenClaw live HTTP bridge"
  );
  expect(world.hotspots[0]?.details).toContain(
    "HTTP bridge status: Preview payload"
  );
  expect(world.hotspots[0]?.details).toContain(
    "HTTP payload: Preview payload available from no-network live-preview execution."
  );
  expect(world.hotspots[0]?.details).toContain(
    "HTTP source dispatch: Preview payload from GET Configured via future live request seam?view=plaza&workspace=mii-plaza-client"
  );
  expect(world.hotspots[0]?.details).toContain(
