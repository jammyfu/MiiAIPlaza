import type {
  ...
  PlazaWorldDataRequestExecutionDelegate,
...
export interface OpenClawFetchRunner<TPayload> {
  ...
  networkExecution: OpenClawNetworkExecution<TPayload>;
...
export interface OpenClawRequestDispatchHandler<TPayload> {
  ...
  httpBridge: OpenClawHttpBridge<TPayload>;
...
export function createOpenClawFetchRunnerFactory(
  ...
  const fetchEntry =
    request.fetchEntry ??
    createOpenClawLiveFetchEntry(executionBridge);
  const fetchDispatch =
    request.fetchDispatch ??
  ...
  const executionDelegate =
    request.executionDelegate ??
    createOpenClawLiveExecutionDelegate(fetchExecution);
  const transportImplementation =
    request.transportImplementation ??
    createOpenClawLiveTransportImplementation(executionDelegate);
  const transportRunner =
    request.transportRunner ??
    createOpenClawLiveTransportRunner(transportImplementation);
  const networkExecution =
    request.liveEnabled
      ? createOpenClawLiveStubNetworkExecution(request, envelope, transportRunner)
      : createOpenClawPreviewNetworkExecution(request, envelope, transportRunner);
...
export function createOpenClawPreviewFetchRunner(
  ...
  transportRunner: PlazaWorldDataRequestTransportRunner =
    request.transportRunner ??
    createOpenClawLiveTransportRunner(transportImplementation),
  networkExecution: OpenClawNetworkExecution<OpenClawLiveResponsePayload> =
    createOpenClawPreviewNetworkExecution(request, envelope, transportRunner)
  ...
  return {
    ...
    async run(now: Date | string | number = Date.now()) {
      return networkExecution.execute(now);
    },
  };
}
...
export function createOpenClawLiveFetchEntry(
  executionBridge: PlazaWorldDataRequestExecutionBridge
): PlazaWorldDataRequestFetchEntry {
  return {
    ...
    payloadLabel: executionBridge.payloadLabel,
    sourceExecutionBridgeLabel: executionBridge.sourceExecutionPayloadLabel,
    fetchTargetLabel: "OpenClaw live fetch call",
    runnerMode: executionBridge.runnerMode,
  };
}

export function createOpenClawLiveFetchDispatch(
  fetchEntry: PlazaWorldDataRequestFetchEntry
): PlazaWorldDataRequestFetchDispatch {
  return {
    ...
    payloadLabel: fetchEntry.payloadLabel,
    sourceFetchEntryLabel: fetchEntry.sourceExecutionBridgeLabel,
    dispatchTargetLabel: "OpenClaw live network invocation",
    runnerMode: fetchEntry.runnerMode,
  };
}

export function createOpenClawLiveTransportCall(
  fetchDispatch: PlazaWorldDataRequestFetchDispatch
): PlazaWorldDataRequestTransportCall {
  return {
    ...
    status: fetchDispatch.status,
    payloadLabel: fetchDispatch.payloadLabel,
    sourceFetchDispatchLabel: fetchDispatch.sourceFetchEntryLabel,
    transportTargetLabel: "OpenClaw live transport invocation",
    runnerMode: fetchDispatch.runnerMode,
  };
}
...
export function createOpenClawLiveExecutionDelegate(
  fetchExecution: PlazaWorldDataRequestFetchExecution
): PlazaWorldDataRequestExecutionDelegate {
  return {
    ...
    status: fetchExecution.status,
    payloadLabel: fetchExecution.payloadLabel,
    sourceFetchExecutionLabel: fetchExecution.sourceTransportCallLabel,
    delegateTargetLabel: "OpenClaw live transport implementation",
    runnerMode: fetchExecution.runnerMode,
  };
}

export function createOpenClawLiveTransportImplementation(
  executionDelegate: PlazaWorldDataRequestExecutionDelegate
): PlazaWorldDataRequestTransportImplementation {
  return {
    ...
    status: executionDelegate.status,
    payloadLabel: executionDelegate.payloadLabel,
    sourceExecutionDelegateLabel: executionDelegate.sourceFetchExecutionLabel,
    runnerTargetLabel: "OpenClaw live transport runner",
    runnerMode: executionDelegate.runnerMode,
  };
}

export function createOpenClawLiveTransportRunner(
  transportImplementation: PlazaWorldDataRequestTransportImplementation
): PlazaWorldDataRequestTransportRunner {
  return {
    ...
    status: transportImplementation.status,
    payloadLabel: transportImplementation.payloadLabel,
    sourceTransportImplementationLabel:
      transportImplementation.sourceExecutionDelegateLabel,
    executionTargetLabel: "OpenClaw live network execution",
    runnerMode: transportImplementation.runnerMode,
  };
}

export function createOpenClawLiveNetworkExecutionMetadata(
  transportRunner: PlazaWorldDataRequestTransportRunner
): PlazaWorldDataRequestNetworkExecution {
  return {
    id: "openclaw-live-network-execution",
    label: "OpenClaw live network execution",
    summary:
      "Represents the injectable execution seam selected from the transport runner before actual live HTTP requests are enabled.",
    status: transportRunner.status,
    payloadLabel: transportRunner.payloadLabel,
    sourceTransportRunnerLabel:
      transportRunner.sourceTransportImplementationLabel,
    implementationLabel:
      transportRunner.runnerMode === "live"
    ...
    runnerMode: transportRunner.runnerMode,
  };
}
...
export function createOpenClawLiveHttpBridgeMetadata(
  ...
  return {
    ...
    status: requestDispatch.status,
    payloadLabel: requestDispatch.payloadLabel,
    sourceRequestDispatchLabel: requestDispatch.sourceNetworkExecutionLabel,
    implementationLabel: requestDispatch.implementationLabel,
    runnerMode: requestDispatch.runnerMode,
  };
}
...
export function createOpenClawLiveStubHttpBridge(
  request: PlazaWorldDataRequest,
  envelope: PlazaWorldDataRequestRunnerEnvelope,
  requestDispatch: PlazaWorldDataRequestDispatchHandler
): OpenClawHttpBridge<OpenClawLiveResponsePayload> {
...
export function createOpenClawPreviewRequestDispatchHandler(
  ...
  const httpBridge = createOpenClawPreviewHttpBridge(request, envelope, metadata);
  return {
    metadata,
    httpBridge,
    async dispatch(now: Date | string | number = Date.now()) {
      return httpBridge.bridge(now);
    },
  };
}
...
export function createOpenClawLiveStubRequestDispatchHandler(
  ...
  const metadata =
    request.requestDispatch ??
    createOpenClawLiveRequestDispatchMetadata(networkExecution);
  const httpBridge = createOpenClawLiveStubHttpBridge(request, envelope, metadata);
  return {
    metadata,
    httpBridge,
    async dispatch(now: Date | string | number = Date.now()) {
...
export function createOpenClawLiveStubNetworkExecution(
  request: PlazaWorldDataRequest,
  envelope: PlazaWorldDataRequestRunnerEnvelope,
  transportRunner: PlazaWorldDataRequestTransportRunner
): OpenClawNetworkExecution<OpenClawLiveResponsePayload> {
